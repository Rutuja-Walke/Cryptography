...
vector<int> sw1; // result of second sw 
...
vector<int> name_thisfun(vector<int> &c)
{
...
vector<int> sw2_operation(vector<int> &IP)
{
    ...
    //swapping left 4 and right 4 bits
    vector<int> sw3;
    for(int i=0; i<4; i++)
    {
        sw3.push_back(ipr[i]);
    ...
    for(int i=0; i<4; i++)
    {
        sw3.push_back(left[i]);
    }

    cout<<"\n\nResult after swapping:\n";
    print_fun(sw3, sw3.size());
    return sw3;
}

//second time swap operation using key1
vector<int> sw1_operation(vector<int> &IP)
{
   ...
   /* cout<<"\n\n\n";
    ...
    {
        int temp = sw1[ip_inverse[i]-1];
        ans.push_back(temp);
    }

    cout<<"\n\n\nOriginal Text after decryption:\n";
    print_fun(ans, ans.size());
...
int main(){
    ...
    vector<int> a = name_thisfun(cipher);
    vector<int> b = sw2_operation(a);
    vector<int> c = sw1_operation(b);
